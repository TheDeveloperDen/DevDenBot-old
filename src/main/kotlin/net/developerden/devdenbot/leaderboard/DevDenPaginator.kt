...
class DevDenPaginator<T> constructor(
    ...
    fun display(channel: MessageChannel, page: Int = 0) = showMessage(page) { channel.sendMessageEmbeds(it) }

    private fun update(msg: Message, page: Int) = showMessage(page) { msg.editMessageEmbeds(it) }

    private fun showMessage(page: Int, showAction: (MessageEmbed) -> MessageAction) =
            showAction.invoke(render(page)).queue { addReactions(it); addReactionListener(it, page) }
    ...
    private fun addReactions(msg: Message) =
        RestAction.allOf(
